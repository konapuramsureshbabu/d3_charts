...
export function drawMultiColumnChart(options, propsData, styles) {
  ...
  const svg = d3
    ...
    // .style("background-image", `url('https://source.unsplash.com/500x400/?nature')`)
  ...
  function chart() {
    ...
    bars
      ...
      .attr(
        "x",
        (d) =>
          (x.bandwidth() / enabledLegendKeys.length) *
          enabledLegendKeys.indexOf(d.dimension)
      )
      .attr("width", x.bandwidth() / enabledLegendKeys.length)
  ...
  function updateScales(styles) {
    ...
    y.domain([minNegativeValue, maxPositiveValue]);
    addAxes(svg, x, y, height, styles, defaultStyle);
  }
  ...
  function addAxes(svg, x, y, height, styles, defaultStyle) {
    ...
    xAxis
      ...
      .attr(
        "transform",
        `rotate (${styles?.xAxis?.tickLabel?.rotation?.value || defaultStyle?.axisLabels?.x?.tickLabelOrientation || 0})`
      )
      .style(
        "text-anchor",
