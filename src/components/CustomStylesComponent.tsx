...
const CustomStyles: React.FC<CustomStylesProps> = ({ data, styles }) => {
  ...
  const stylesJSON = JSON?.parse(codeStyles);
    ...
    const handleRunChart = () => {
      try {
        ...
        const chartTypeConfig =
          chartConfig[params.id as keyof typeof chartConfig];
        ...
        if (chartTypeConfig) {
          ...
          console.error("Invalid chart type:", params.id);
        }
      } catch (error) {
